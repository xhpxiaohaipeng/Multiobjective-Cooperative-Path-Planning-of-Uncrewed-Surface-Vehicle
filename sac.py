...
class SAC_self(parl.Algorithm):
    def __init__(self,
        ...
        if adaptive_alpha:
            ...
            self.target_entropy = paddle.to_tensor([-model.action_dim], dtype='float32', stop_gradient=False)#paddle.tensor(-model.action_dim, dtype=float, requires_grad=True)
            # We learn log_alpha instead of alpha to ensure exp(log_alpha)=alpha>0
            self.log_alpha = paddle.to_tensor([np.log(alpha)], dtype='float32', stop_gradient=False)
            self.alpha_optim = paddle.optimizer.Adam(parameters=[self.log_alpha], learning_rate=critic_lr)#$torch.optim.Adam([self.log_alpha], lr=critic_lr)
